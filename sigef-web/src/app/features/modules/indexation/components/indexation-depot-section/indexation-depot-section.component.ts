...
import { DepotTitreFoncierService } from '../../../../../data/modules/gestion-dossiers/services/depot-titre-foncier.service';
import { DepotTitreFoncier } from '../../../../../data/modules/gestion-dossiers/models/DepotTitreFoncier';
...
@Component({
  selector: 'app-indexation-depot-section',
  templateUrl: './indexation-depot-section.component.html',
  styleUrl: './indexation-depot-section.component.scss'
})
export class IndexationDepotSectionComponent {
  ...
  showSuppressionDepotTitreFoncierModal: boolean = false
  suppressionDepotTitreFoncierForm: FormGroup = new FormGroup({
  ...
  nouveauDepotForm: FormGroup = new FormGroup({
    ...
    statut: new FormControl(StatutsDepot.VALIDE, [Validators.required]),
  ...
  status = [
    { id: StatutsDepot.VALIDE, libelle: 'Valide' },
    { id: StatutsDepot.ANNULE, libelle: 'Annulé' },
  ]

  onStatutChange(): void {
    if (this.nouveauDepotForm.get('statut')!.value != StatutsDepot.VALIDE) {
      this.nouveauDepotForm.controls['informationsStatut'].setValidators([Validators.required])
      this.nouveauDepotForm.controls['informationsStatut'].updateValueAndValidity()
    }
    else {
      this.nouveauDepotForm.controls['informationsStatut'].setValidators([])
      this.nouveauDepotForm.controls['informationsStatut'].updateValueAndValidity()
    }
  }
  ...
  constructor(
    ...
    private depotTitreFoncierService: DepotTitreFoncierService,
    // Parties prenantes
    ...
    // this.nouveauDepotForm.valueChanges.subscribe(value => {
    //   // console.log("Modifié: ", value == this.progressionTacheIndexation?.donneeIndexation?.depot)
    // });
  }
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes['formulaireEtatActif']) {
      if (this.formulaireEtatActif) {
        ...
        this.nouveauDepotForm.enable(); // Désactiver le formulaire
      } else {
        this.metadonneesForm.disable(); // Réactiver le formulaire
        this.nouveauDepotForm.disable(); // Réactiver le formulaire
      }
    }

    if (changes['pageIndexation']) {
      console.log(this.progressionTacheIndexation)
      this.resetNouveauDepotForm()
      this.initnouveauDepotForm()
  ...
  private getProgressionTacheIndexationValue(removeIds: boolean): ProgressionTacheIndexation {
    if (removeIds) {
      this.nouveauDepotForm.removeControl('id');

      (this.alienateurs.controls as FormGroup[]).forEach((group: FormGroup) => {
        group.removeControl('id');
      });
      (this.acquereurs.controls as FormGroup[]).forEach((group: FormGroup) => {
        group.removeControl('id');
      });
      (this.depotsTitresFonciers.controls as FormGroup[]).forEach((group: FormGroup) => {
        group.removeControl('id');
      });
    }
    ...
    depot.partiesPrenantes = this.getPartiesPrenantes(removeIds)
  ...
  validerSaisie(): void {
    ...
    if (this.nouveauDepotForm.valid && this.metadonneesForm.valid) {
        ...
        this.progressionTacheIndexationService.update(progressionTacheIndexation)
          .subscribe({
            next: (value: ProgressionTacheIndexation) => {
              ...
              let depot = progressionTacheIndexation.donneeIndexation?.depot
              if (depot && depot.id != null) {
  ...
  initnouveauDepotForm(): void {
    ...
    if (this.progressionTacheIndexation) {
      ...
      console.log("Init nouveau depot form: ", this.tacheIndexation)
      ...
      let depot: Depot | undefined = this.progressionTacheIndexation?.donneeIndexation?.depot
      if (depot) {
        this.nouveauDepotForm.patchValue(depot)
        console.log("Init: ", depot)

        depot.partiesPrenantes?.forEach((partiePrenante: PartiePrenante) => {
        ...
        depot.depotsTitresFonciers?.forEach((depotTitreFoncier: DepotTitreFoncier) => {
          this.addDepotTitreFoncier(false, depotTitreFoncier)
        })
      }
    }
    else {
  ...
  resetNouveauDepotForm(): void {
    this.nouveauDepotForm.reset()
    for (let index = 0; index < this.alienateurs.length; index++) {
      this.alienateurs.removeAt(index)
    }
    for (let index = 0; index < this.acquereurs.length; index++) {
      this.acquereurs.removeAt(index)
    }
    for (let index = 0; index < this.depotsTitresFonciers.length; index++) {
  ...
  addDepotTitreFoncier(removeIds: boolean, value?: any): void {
    let depotTitreFoncierFormGroup: FormGroup = new FormGroup({
      numeroTitreFoncier: new FormControl(value ? value.numeroTitreFoncier : null, []),
    })
    ...
    if (!this.formulaireEtatActif) {
      depotTitreFoncierFormGroup.disable()
    }
  ...
  removeDepotTitreFoncier(index: number): void {
    if (this.progressionTacheIndexation) {
      this.suppressionDepotTitreFoncierForm.get('index')?.setValue(index)
      this.suppressionDepotTitreFoncierForm.get('id')?.setValue(this.depotsTitresFonciers.at(index).get('id')?.value)

      this.openSuppressionDepotTitreFoncierModal()
    }
    else {
  ...
  supprimerDepotTitreFoncier(): void {
    if (this.suppressionDepotTitreFoncierForm.valid) {
      let depotTitreFoncierId: string = this.suppressionDepotTitreFoncierForm.get('id')?.value
      console.log(depotTitreFoncierId)

      this.depotTitreFoncierService.delete(depotTitreFoncierId)
        .subscribe({
          next: () => {
            let depotTitreFoncierIndex: number = this.suppressionDepotTitreFoncierForm.get('index')?.value

            this.removeDepotTitreFoncierAtIndex(depotTitreFoncierIndex)
            this.closeSuppressionDepotTitreFoncierModal()
            this.notificationsHandlerService.addNotification({type: TypesNotificationAlert.SUCCESS, title: 'Opération réussie', description: 'Ce numéro de titre foncier a été supprimé avec succès'})
          },
          error: (err: HttpErrorResponse) => {
            console.log(err)
  ...
  // Modals
  openSuppressionDepotTitreFoncierModal(): void {
    this.showSuppressionDepotTitreFoncierModal = true
  }

  closeSuppressionDepotTitreFoncierModal(): void {
    this.suppressionDepotTitreFoncierForm.reset()
    this.showSuppressionDepotTitreFoncierModal = false
  }
