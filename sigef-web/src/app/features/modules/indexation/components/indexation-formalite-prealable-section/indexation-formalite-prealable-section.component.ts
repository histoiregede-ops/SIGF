...
@Component({
  selector: 'app-indexation-formalite-prealable-section',
  templateUrl: './indexation-formalite-prealable-section.component.html',
  styleUrl: './indexation-formalite-prealable-section.component.scss'
})
export class IndexationFormalitePrealableSectionComponent implements OnInit, OnChanges {
  ...
  nouvelleFormalitePrealableForm: FormGroup = new FormGroup({
    ...
    statut: new FormControl(StatutsFormalitePrealable.VALIDEE, [Validators.required]),
    informationsStatut: new FormControl(null, []),
    dateDeDepot: new FormControl(null, []),
    ...
    //   // natureTypeImmeubleId: new FormControl(null, []),
    //   // natureEtatImmeubleId: new FormControl(null, []),
    //   contenanceEnHectare: new FormControl(null, []),
    //   contenanceEnAre: new FormControl(null, []),
    //   contenanceEnCentiare: new FormControl(null, []),
    //   // valeurVenale: new FormControl(null, []),
    //   // valeurLocative: new FormControl(null, []),
    // }),
    ...
    // nombrePiecesJointes: new FormControl(null, []),
    // dateSommationDepotDePieces: new FormControl(null, []),
    // dateDepotDepotDePieces: new FormControl(null, []),
    publicationDemandes: new FormGroup({
      id: new FormControl(null, []),
      //   dateTransmissionInsertionJournal: new FormControl(null, []),
      //   dateInsertionJournal: new FormControl(null, []),
      //   dateTransmissionAffichageAudienceTribunal: new FormControl(null, []),
      dateAffichageAudienceTribunal: new FormControl(null, []),
      //   dateEnvoiAffichagePublication: new FormControl(null, []),
      //   dateAccuseReceptionAffichagePublication: new FormControl(null, []),
      //   dateEnvoiNotificationsIndividuelles: new FormControl(null, []),
      //   dateRetourAccusesReceptionNotificationsIndividuelles: new FormControl(null, []),
      //   dateNormaleClotureDelais: new FormControl(null, []),
      //   propagationRaisonAbsencesClotureDelais: new FormControl(null, []),
    }),
    bornage: new FormGroup({
      id: new FormControl(null, []),
      //   dateInsertionJournalOfficiel: new FormControl(null, []),
      //   dateAvisIndividuels: new FormControl(null, []),
      dateProcesVerbal: new FormControl(null, []),
    }),
    // procedureJudiciaire: new FormGroup({
    //   id: new FormControl(null, []),
    //   dateJugementEnPremiereInstance: new FormControl(null, []),
    //   dateRemiseExpeditionEnPremiereInstance: new FormControl(null, []),
    //   dateRecoursEnPremiereInstance: new FormControl(null, []),
    //   dateArreEnAppel: new FormControl(null, []),
    //   dateRemiseExpeditionEnAppel: new FormControl(null, []),
    //   dateRecoursEnAppel: new FormControl(null, []),
    //   dateArreEnCassation: new FormControl(null, []),
    //   dateRemiseExpeditionEnCassation: new FormControl(null, []),
    // }),
    // nombreCopiesTitresFonciersEtablies: new FormControl(null, []),
    // nombreCertificatsInscriptionEtablis: new FormControl(null, []),
  ...
  onStatutChange(): void {
    if (this.nouvelleFormalitePrealableForm.get('statut')!.value != StatutsFormalitePrealable.VALIDEE) {
      this.nouvelleFormalitePrealableForm.controls['informationsStatut'].setValidators([Validators.required])
      this.nouvelleFormalitePrealableForm.controls['informationsStatut'].updateValueAndValidity()
    }
    else {
      this.nouvelleFormalitePrealableForm.controls['informationsStatut'].setValidators([])
      this.nouvelleFormalitePrealableForm.controls['informationsStatut'].updateValueAndValidity()
    }
  }
  ...
  constructor(
    ...
    // this.nouvelleFormalitePrealableForm.valueChanges.subscribe(value => {
    //   // console.log("Modifié: ", value == this.progressionTacheIndexation?.donneeIndexation?.formalitePrealable)
    // });
  }
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes['formulaireEtatActif']) {
      if (this.formulaireEtatActif) {
        ...
        this.nouvelleFormalitePrealableForm.enable(); // Désactiver le formulaire
      } else {
        this.metadonneesForm.disable(); // Réactiver le formulaire
        this.nouvelleFormalitePrealableForm.disable(); // Réactiver le formulaire
      }
    }

    if (changes['pageIndexation']) {
      console.log(this.progressionTacheIndexation)
      this.resetNouvelleFormalitePrealableForm()
      this.initNouvelleFormalitePrealableForm()
  ...
  private getProgressionTacheIndexationValue(removeIds: boolean): ProgressionTacheIndexation {
    if (removeIds) {
      this.nouvelleFormalitePrealableForm.removeControl('id');
      (this.nouvelleFormalitePrealableForm.get('informationsPropriete') as FormGroup | null)?.removeControl('id');
      (this.nouvelleFormalitePrealableForm.get('situationPropriete') as FormGroup | null)?.removeControl('id');
      (this.nouvelleFormalitePrealableForm.get('publicationDemandes') as FormGroup | null)?.removeControl('id');
      (this.nouvelleFormalitePrealableForm.get('bornage') as FormGroup | null)?.removeControl('id');
      (this.nouvelleFormalitePrealableForm.get('procedureJudiciaire') as FormGroup | null)?.removeControl('id');

      (this.requerants.controls as FormGroup[]).forEach((group: FormGroup) => {
        group.removeControl('id');
      });
      (this.proprietaires.controls as FormGroup[]).forEach((group: FormGroup) => {
        group.removeControl('id');
      });
    }
    ...
    formalitePrealable.partiesPrenantes = this.getPartiesPrenantes(removeIds)
    ...
    donneeIndexation.formalitePrealable = formalitePrealable
  ...
  validerSaisie(): void {
    ...
    if (this.nouvelleFormalitePrealableForm.valid && this.metadonneesForm.valid) {
        ...
        this.progressionTacheIndexationService.update(progressionTacheIndexation)
          .subscribe({
            next: (value: ProgressionTacheIndexation) => {
              ...
              let formalitePrealable = progressionTacheIndexation.donneeIndexation?.formalitePrealable
              if (formalitePrealable && formalitePrealable.id != null) {
                this.formalitePrealableService.update(formalitePrealable)
                  .subscribe({
                    next: (value) => {
                      this.resetNouvelleFormalitePrealableForm()
  ...
  initNouvelleFormalitePrealableForm(): void {
    if (this.progressionTacheIndexation) {
      // Informations
      this.getNaturesTypesImmeuble()
      this.getNaturesEtatsImmeuble()
      // Situations
      this.getRegions()
      this.getPrefectures()
      this.getCommunes()
      ...
      console.log("Init nouvelle formalité préalable form: ", this.tacheIndexation)
      ...
      let formalitePrealable: FormalitePrealable | undefined = this.progressionTacheIndexation?.donneeIndexation?.formalitePrealable
      if (formalitePrealable) {
        this.nouvelleFormalitePrealableForm.patchValue(formalitePrealable)
        console.log("Init: ", formalitePrealable)

        formalitePrealable.partiesPrenantes?.forEach((partiePrenante: PartiePrenante) => {
  ...
  resetNouvelleFormalitePrealableForm(): void {
    this.nouvelleFormalitePrealableForm.reset()
    for (let index = 0; index < this.requerants.length; index++) {
      this.requerants.removeAt(index)
    }
    for (let index = 0; index < this.proprietaires.length; index++) {
      this.proprietaires.removeAt(index)
    }
